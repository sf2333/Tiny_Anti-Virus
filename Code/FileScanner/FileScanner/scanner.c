...
--*/
{
    ...
    DbgPrint("[主动防御]安装FileScanner驱动成功!\n");
...
--*/
{
    ...
    DbgPrint( "成功连接!!! scanner.sys --- connected, port=0x%p\n", ClientPort );
...
--*/
{
    ...
    DbgPrint("[主动防御]卸载FileScanner驱动成功!\n");
        ...
        if (Data->Iopb->Parameters.Write.Length != 0) {
            ...
            try  {
                ...
                if (!NT_SUCCESS(status)) {
                    ...
                    DbgPrint(" FltGetFileNameInformation失败！\n");
                    return FLT_PREOP_SUCCESS_NO_CALLBACK;
                }

                // 将 FileNameInfo->Name 复制到 notification->FilePath 中
                wcsncpy(notification->FilePath, FileNameInfo->Name.Buffer, min(FileNameInfo->Name.Length, FILE_PATH_MAX_LENGTH));
                notification->FilePath[min(FileNameInfo->Name.Length, FILE_PATH_MAX_LENGTH) / sizeof(WCHAR)] = L'\0'; // 添加字符串终止符
